...
if args.const== 1: trait_tag = "_const_"
if args.out != "": trait_tag = "%s_%s_" % (trait_tag, args.out)
if max_age < np.inf or min_age > 0:
	out_file_name = "%s/%s%s%s-%s.log" % (output_wd,name_file,trait_tag,max_age,min_age)
else:
	out_file_name = "%s/%s%s.log" % (output_wd,name_file,trait_tag)
...
Y_vecA = np.array([np.random.normal(0,0.1,i) for i in trait_categories_list]) #
...
likA = -np.inf
prior_Y = [sum(prior_normal(Y_vecA[i],m=0,sd=std_HP[i])) for i in range(n_traits)]
...
while iteration <= n_iterations:
	...
	# lineage-sp rates
	sum_rates,prior_Y,prior_Tau,prior_Beta = np.zeros(n_lineages),0,0,0
	for i in range(n_traits):
