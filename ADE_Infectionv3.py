...
#		k,h = update_poisson_proposal(kA)
#		if h > log(np.random.random()):
#			kA = k
#		#if i % 100 ==0: print kA
#	print j, kA
#	finalK.append(kA)
...
def BDwwteDISCRETE(args,updated_ext, D_lik):
	...
	if useInfectionModel==0:
		...
		# lik speciation
		birth_rates, Kvec = birth_rates_infection(Dt, l0,gam,thres,k0_t,km_t,mt=1./longevity)
		birth_rates[birth_rates<=0] = small_number
	...
	return [lik, birth_rates, 1./longevity, Kvec]


def calc_prior(args):
	#args=  [l0,  gam,	thres,   k0_t,  km_t,	W_shape,  W_scale]
	p = prior_gamma(args[4]+max_obs_diversity,a=shapeG,s=scaleG,l=locG)
	return p
...
# read data
tbl = np.loadtxt("all_bands_1.tsv",skiprows=1)
ts = tbl[:,2]
te = tbl[:,3]
...
max_obs_diversity = max(Dt)
l0      = random.uniform(0.25,0.6)
gam     = random.uniform(0.05,0.3)
thres   = random.uniform(0.01,0.05)
km_t = 1000. # extra Kmax beyond max Dt
k0_t = 1000. # extra K0 beyond min Dt
W_shape = 1.        # extinction parameters (discrete Weibull model)
W_scale = 1./0.20  # extinction parameters (discrete Weibull model)

# prior on Kmax - gamma(shape,scale,location) 
shapeG = 1.
scaleG = max_obs_diversity
locG   = 0 
...
head =["it","lik","prior","l0","gam","thres","k0","kMax","W_shape","W_scale","m0"]
...
while iteration != 50000000:
	...
	if rr[0] < 0.1 or useInfectionModel==0:
		if rr[1]<0.7: # update extinction
			...
			res = update_multiplier_proposal_vec(args,d=1.1,f=np.array([1,0,0,0,0,0,0.]))	
		[args, hastings] = res
	elif rr[0] < 0.35:
	...
	if iteration % sampling_freq==0:
		...
		print iteration, likA, argsO
		l= [iteration, likA, priorA] + list(argsO) + [death_rate] + list(birth_rates) + list(Kvec)
		wlog.writerow(l)
		logfile.flush()
		os.fsync(logfile)
