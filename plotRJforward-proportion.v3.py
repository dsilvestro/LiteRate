...
def get_marginal_rates(f_name,start_age,end_age,nbins=0,burnin=0.2):
    print(f_name,start_age,end_age)
    ...
    print(marginal_rates_list.shape)
...
def get_r_plot(res,col,parameter,min_age,max_age,plot_title,plot_log,run_simulation=1,BFs=None):
    ...
    return out_str, BFs, [counts_bf2, counts_bf6]
...
def pretty_ggplot(resS,resE,div_log):
    ...
    if np.min(tbl[:,2]) > 0:
        ...
        if args.proportion==0: emp_net=emp_birth-emp_death
        else: emp_net=tbl[:,0]/tbl[:,1]    
...
def make_joint_event_series(sp_events, ex_events,mean_rates):
    """
    Creates a series marking joint events where an event occurs in both input arrays.

    Parameters:
        sp_events (np.array): The first array of events (special events).
        ex_events (np.array): The second array of events (external events).

    Returns:
        np.array: An array with 'Event' marking where joint events occur and None elsewhere.
    """
    # Initialize the new array and flags
    event_happened_in_both = []
    flag_sp_events = False
    flag_ex_events = False
    i=0
    # Jointly iterate over both arrays
    for val_sp, val_ex in zip(sp_events, ex_events):
        # Update flags if an event occurs in either array
        if not np.isnan(val_sp):
            flag_sp_events = True
        if not np.isnan(val_ex):
            flag_ex_events = True

        # Check if events occurred in both arrays
        if flag_sp_events and flag_ex_events:
            event_happened_in_both.append(mean_rates[i])
            # Reset flags
            flag_sp_events = False
            flag_ex_events = False
        else:
            event_happened_in_both.append(np.nan)
        i+=1

    # Convert the result to a numpy array
    return np.array(event_happened_in_both)


def plot_net_rate(resS,resE,col,min_age,max_age,nbins,plot_title,div_log,burnin=.2,BFs=None,sp_bfcounts=None,ex_bfcounts=None):
    tbl=np.loadtxt(div_log, skiprows=1)
    num_emp=tbl[:,0]
    denom_emp=tbl[:,1]
    ...
    if args.proportion==0: 
        ...
        marginal_rates_list = resS_marginal_rate[0:max_indx,:] / resE_marginal_rate[0:max_indx,:]
        marginal_rates_list = uniform_filter1d(marginal_rates_list, size=3, axis=1, mode='nearest')
    mean_rates= np.mean(marginal_rates_list,axis=0)
    min_rates,max_rates=[],[]
    for i in range(nbins):
        hpd = calcHPD(marginal_rates_list[:,i],0.95)
    ...
    if TBP == True:
        ...
        minXaxis,maxXaxis= max_age,min_age
        time_lab = "AD"
    mean_rates=mean_rates[::-1]; min_rates=min_rates[::-1]; max_rates=max_rates[::-1]
    print("DENOM EMP", denom_emp)
    for i,y in enumerate(denom_emp):
        print(i,y)
        if y==0:
            print(i,y)
            mean_rates[i]=0; min_rates[i]=0; max_rates[i]=0
    print("MAX:",np.nanmax(mean_rates),np.nanmax(min_rates),np.nanmax(max_rates))
    ...
    out_str += "\nabline(h=0,lty=2)\n"
    if args.proportion!=0:
        print("SPBF_counts",mean_rates)
        print("SPBF_counts",sp_bfcounts)
        print("SPEX_counts",ex_bfcounts)
        net_bf2=make_joint_event_series(sp_bfcounts[0],ex_bfcounts[0],mean_rates)
        net_bf6=make_joint_event_series(sp_bfcounts[1],ex_bfcounts[1],mean_rates)
        print("RES2",net_bf2)
        print("RES6",net_bf6)        
        out_str += print_R_vec("\nnet_BF2",net_bf2)
        out_str += print_R_vec("\nnet_BF6",net_bf6)
...
def plot_marginal_rates(path_dir,name_tag="",bin_size=1.,burnin=0.2,min_age=0,max_age=0,logT=0,combine=0):
    ...
    files=[f for f in files if '_COMBINED' not in f]
    files.sort()
    stem_file=files[0]
    ...
    for mcmc_file in files:
        if 2>1: #try:
            ...
            spec_str,BFs,bf_counts_sp=get_r_plot(resS,col=colors[0],parameter="Birth rate",min_age=min_age_t,max_age=max_age_t,plot_title=name_file,plot_log=logT)
            r_str+=spec_str
            # ex file
            ...
            ex_str,_,bf_counts_ex = get_r_plot(resE,col=colors[1],parameter="Death rate",min_age=min_age_t,max_age=max_age_t,plot_title="",plot_log=logT,run_simulation=0,BFs=BFs)

            
            #net rate
            r_str += plot_net_rate(resS,resE,col=colors[2],min_age=min_age_t,max_age=max_age_t,nbins=nbins,
                                   plot_title='Net Rate' if args.proportion==0 else 'Ratio of Rates',
                                   div_log=mcmc_file.replace("mcmc.log","div.log"),sp_bfcounts=bf_counts_sp,ex_bfcounts=bf_counts_ex)
...
p.add_argument('-proportion', help='Changes net rate plot into ratio between rates', default=False, action='store_true')
