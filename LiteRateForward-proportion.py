...
def BDI_partial_lik(L_acc_vec,M_acc_vec):
    ...
    if args.proportion==1:
        #print(model_BDI,I.shape,kn.shape,M.shape,kd.shape)

        lik_Inum = sum(log(I[kn>0])*Uk[kn>0] - (I[kn>0])*Tk[kn>0])
        lik_Iden = sum(log(M[kd>0])*Dk[kd>0] - (M[kd>0])*Tk[kd>0])
        lik = lik_Inum + lik_Iden

        #lik_BI = sum(log(L[k>0]*k[k>0]+I[k>0])*Uk[k>0] - (L[k>0]*k[k>0]+I[k>0])*Tk[k>0])
        #lik_D = sum(log(M*k)*Dk -(M*k*Tk))

    else:
        #print(model_BDI,I.shape,k.shape)
        lik = Uk[k>0]*log(k[k>0]*L[k>0]+I[k>0]) + Dk[k>0]*log(M[k>0]*k[k>0]) - Tk[k>0]*(k[k>0]*(L[k>0]+M[k>0])+I[k>0])
        lik=sum(lik)
    #quit()
    return lik # lik_BI + lik_D
...
p.add_argument('-proportion',   type=int, help='turns ts and te into independent immigration processes', default=0, metavar=0)
...
if args.proportion==1: 
    model_BDI=1
    out_name = "_PR"
out_name = out_name + '_seed'+ str(args.seed) + args.out
...
####### Parse DATA #######
f = args.d
t_file=np.genfromtxt(f,delimiter='\t', skip_header=1)
if t_file.shape[1]==4:
...
if args.proportion==0:te = te + args.death_jitter
start_time = np.min([np.min(ts[~np.isnan(ts)]),np.min(te[~np.isnan(te)])])
end_time = np.max([np.max(ts[~np.isnan(ts)]),np.max(te[~np.isnan(te)])])
if args.proportion==1: end_time+=args.death_jitter
...
if args.proportion==0:
    ...
    k = np.array(br_length_bin)

if args.proportion==1:
    sp_events_bin = pd.Series(ts[~np.isnan(ts)]).value_counts()
    ex_events_bin = pd.Series(te[~np.isnan(te)]).value_counts()
    for y in bins:
        if y not in ex_events_bin.index: ex_events_bin[y]=None
        elif y not in sp_events_bin.index: sp_events_bin[y]=0
        if y not in sp_events_bin.index: sp_events_bin[y]=None
    
    sp_events_bin=sp_events_bin.sort_index().interpolate().to_numpy()[:-2]
    ex_events_bin=ex_events_bin.sort_index().interpolate().to_numpy()[:-2]             
    kn=np.cumsum(sp_events_bin)
    kd=np.cumsum(ex_events_bin)
    print(sp_events_bin,ex_events_bin,kn,kd)
    #write diversity. plotRj uses this
    div_rows = zip(sp_events_bin,ex_events_bin,kn,kd)
    div_logfile.write('sp_events1\tsp_events2\tbr_length1\tbr_length2\n')
    writer = csv.writer(div_logfile,delimiter='\t')
    for row in div_rows:
        writer.writerow(row)
    div_logfile.close()
...
if calc_adequacy and args.proportion==0:
    B_EMP,D_EMP=print_empirical_rates(sp_events_bin,ex_events_bin,br_length_bin)

if calc_adequacy and args.proportion==1:
    B_EMP,D_EMP = sp_events_bin, ex_events_bin
